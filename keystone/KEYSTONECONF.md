...
Format string to use for log messages with context. (string value)  Format string to use for log messages when context is undefined. (string value)
...
Prefix each line of exception output with this format. (string value)  Defines the format string for %(user_identity)s that is used in logging_context_format_string. (string value)
...
Enables or disables publication of error events. (boolean value)  The format for an instance that is passed with the log message. (string value)
...
Interval, number of seconds, of log rate limiting. (integer value)  Maximum number of logged messages per rate_limit_interval. (integer value)
...
The pool size limit for connections expiration policy (integer value)  The time-to-live in sec of idle connections in the pool (integer value)
